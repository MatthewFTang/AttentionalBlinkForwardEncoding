...
tmp = load('../data/testdata_2class.mat');
...
cfg = [];
Xhat = decode_LDA(cfg, decoder, Y_test);

m0 = mean(Xhat(X_test==0));
m1 = mean(Xhat(X_test==1));

% Classify
class = (Xhat > 0);

pCorrect = mean(X_test == class);

% Test and plot
[~, p] = ttest2(Xhat(X_test==0), Xhat(X_test==1));

edges = linspace(min(Xhat), max(Xhat), 15);
n0 = histc(Xhat(X_test==0), edges);
n1 = histc(Xhat(X_test==1), edges);
